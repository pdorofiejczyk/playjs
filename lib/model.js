(function($) {
	...
	var renderCreate = function(templateName, data, chunkSize) {
		var template = $("[type='html/"+templateName+"']").html(),
			promose = $.observable({}),
			i = 0;

		promise.interval = setInterval(100, function() {
			var $rendered = $();
			for(var j = 0; j < chunkSize && i+i < data.length; j++) {
				i = i+j;
				$rendered = $rendered.add($.render(template, data[i]));
			}

			promise.trigger('chunkRendered', $rendered);
			
			if(i === data.length) {
				promise.trigger('rendered');
			}
		});
